...
function mostrarTareas() {
    ...
    for (var i = 0; i < tareas.length; i++) {
        ...
        if (!tarea.completada) {
            html += '<button class="btn-completar" onclick="completarTarea(' + tarea.id + ')">✓ Completar</button>';
        }
        
        // Botón de eliminar
        html += '<button class="btn-eliminar" onclick="eliminarTarea(' + tarea.id + ')">🗑️ Eliminar</button>';
        html += '</div>';
        html += '</div>';
